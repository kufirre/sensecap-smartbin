// Implement example's oai_wifi_init() and oai_wifi() using SenseCAP UI hooks
#include <assert.h>
#include <esp_event.h>
#include <esp_log.h>
#include <esp_wifi.h>
...
void oai_wifi(void) {
  ...
  if (strlen((const char *)wifi_config.sta.ssid)) {
    ESP_LOGI(TAG, "Connecting to last WiFi SSID: %s", wifi_config.sta.ssid);
  } else {
...
#else
    ESP_LOGI(TAG, "Please set WIFI_SSID and WIFI_PASSWORD via cmdline, then reboot");
