...
#include "wifi.h"   // provides smartbin_wifi_check_and_switch_mode()
...
static esp_err_t config_post_handler(httpd_req_t *req)
{
    ...
    if (err == ESP_OK && cfg.wifi_ssid[0] != '\0') {
        ...
        smartbin_wifi_check_and_switch_mode();
    }
...
esp_err_t webserver_get_config(smartbin_config_t *config)
{
    return config_read(config);
}

esp_err_t webserver_set_config(const smartbin_config_t *config)
{
    return config_write(config);
}

esp_err_t webserver_init_static_assets(void)
{
...
esp_err_t webserver_start_mdns(void)
{
    if (s_mdns_started) {
...
esp_err_t webserver_stop_mdns(void)
{
    if (!s_mdns_started) {
        return ESP_OK;
    }
