...
static smartbin_wifi_state_t g_wifi_state = SMARTBIN_WIFI_STATE_DISCONNECTED;
    ...
    if (event_base == WIFI_EVENT) {
        switch (event_id) {
            case WIFI_EVENT_STA_START:
                ...
                if (g_wifi_state != SMARTBIN_WIFI_STATE_CONNECTED && g_wifi_state != SMARTBIN_WIFI_STATE_CONNECTING) {
                    esp_err_t connect_result = esp_wifi_connect();
                    if (connect_result == ESP_OK) {
                        g_wifi_state = SMARTBIN_WIFI_STATE_CONNECTING;
                    } else {
            ...
            case WIFI_EVENT_STA_DISCONNECTED:
                ...
                g_wifi_state = SMARTBIN_WIFI_STATE_DISCONNECTED;
            ...
            case WIFI_EVENT_AP_START:
                ...
                g_wifi_state = SMARTBIN_WIFI_STATE_AP_MODE;
                
                // Start mDNS for configuration portal access (AP mode)
                smartbin_webserver_start_mdns();
...
static void wifi_monitoring_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (g_wifi_state == SMARTBIN_WIFI_STATE_DISCONNECTED) {
...
esp_err_t smartbin_wifi_connect(void) {
    ...
    smartbin_webserver_init();
    
    // Try to read stored WiFi credentials
    smartbin_config_t config = {};
    if (smartbin_webserver_get_config(&config) == ESP_OK && strlen(config.wifi_ssid) > 0) {
...
// Legacy functions removed - use smartbin_wifi_* functions directly